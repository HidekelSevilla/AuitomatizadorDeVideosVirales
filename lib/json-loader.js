...
const ENGLISH_NOVELA_VOICE_ID = "933563129e564b19a115bedd57b7406a";
const MANHWA_DEFAULT_TTS_MODEL_ID = "eleven_multilingual_v2";
const MANHWA_DEFAULT_ELEVENLABS_SPEED = 1.0;
const MANHWA_DEFAULT_VOICE_SETTINGS = Object.freeze({
  stability: 0.5,
...
function humanizeFlowToken(value) {
  const text = cleanString(value).replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim();
  return text ? text.charAt(0).toUpperCase() + text.slice(1) : "Referencia";
}
...
function v7ScenarioCamerasCompatible(current, reference) {
  if (!current || typeof current !== "object" || !reference || typeof reference !== "object") return false;
  return cleanString(current.elevation).toUpperCase() === cleanString(reference.elevation).toUpperCase()
    && cleanString(current.viewpoint).toUpperCase() === cleanString(reference.viewpoint).toUpperCase()
    && finiteNumber(current.azimuth_deg) && finiteNumber(reference.azimuth_deg)
    && angularDistance(current.azimuth_deg, reference.azimuth_deg) <= 20
    && finiteNumber(current.lens_mm) && finiteNumber(reference.lens_mm)
    && Math.abs(current.lens_mm - reference.lens_mm) <= 15
    && finiteNumber(current.roll_deg) && finiteNumber(reference.roll_deg)
    && Math.abs(current.roll_deg - reference.roll_deg) <= 10;
}
...
function manhwaAssetDef(v) {
  if (typeof v === "string") return { mode: "existing", asset: v.trim(), prompt: "", referenceAssets: [], referenceKey: "", referenceDisabled: false };
  if (!v || typeof v !== "object" || Array.isArray(v)) return { mode: "", asset: "", prompt: "", referenceAssets: [], referenceKey: "", referenceDisabled: false };
  ...
  return {
    ...
    // Algunas poses extremas (cadaver tendido, cuerpo cargado, vista cenital) son
    // sistematicamente anuladas por la placa frontal base. Permitir excluirla de
    // forma explicita conserva el default seguro para todas las demas variantes.
    referenceDisabled: v.reference_pose === false || v.reference_view === false,
  };
}
...
function runtimeToken(value, fallback) {
  const token = cleanString(value).toLowerCase().replace(/[^a-z0-9_-]+/g, "_").replace(/^_+|_+$/g, "");
  return token || fallback;
}

// V6 conserva una escena editorial por pagina, pero sus paginas compuestas necesitan una o
// varias generaciones reales. El runtime las expande a tareas internas y deja una tarea final
// de composicion. V7 no usa este mecanismo: Grok dibuja directamente la pagina completa.
function expandManhwaRuntimeScenes(rawJson, rawScenes, errors) {
  if (rawJson?.project?.preset !== "manhwa") return rawScenes;
  const isV7 = rawJson?.v7_contract && typeof rawJson.v7_contract === "object"
    && !Array.isArray(rawJson.v7_contract);
  // V7 vuelve al contrato editorial nativo de Grok: una escena panel equivale a una
  // solicitud y a un JPG 9:16 ya compuesto por el modelo. No se crean celdas internas
  // ni una tarea parent de composicion. El adaptador de slots queda reservado a V6.
  if (isV7) return rawScenes;
  const adapterEnabled = rawJson?.v6_contract?.runtime_adapter?.page_blueprint_slots_integrated === true;
  const contractName = "v6_contract";
  const expanded = [];
  for (const [sceneIndex, raw] of rawScenes.entries()) {
    const parentId = cleanString(raw?.id) || cleanString(raw?.scene_id);
    const page = raw?.visual?.page_blueprint;
    const composed = page && typeof page === "object" && !Array.isArray(page)
      && cleanString(page.template) && (isV7 || cleanString(page.template) !== "FULL_BLEED");
    if (!composed) {
      expanded.push(raw);
      continue;
    }
    if (!adapterEnabled) {
      errors.push(`scene[${sceneIndex}] (${parentId || "?"}): page_blueprint requiere ${contractName}.runtime_adapter.page_blueprint_slots_integrated=true.`);
      expanded.push(raw);
      continue;
    }
    const slots = Array.isArray(page.slots) ? page.slots : [];
    if (!slots.length) {
      errors.push(`scene[${sceneIndex}] (${parentId || "?"}): page_blueprint.slots debe ser un array no vacio.`);
      expanded.push(raw);
      continue;
    }
    const cellIds = [];
    for (const [slotIndex, slot] of slots.entries()) {
      const slotId = cleanString(slot?.id) || `slot_${slotIndex + 1}`;
      const cellId = `${parentId}__cell_${runtimeToken(slotId, String(slotIndex + 1))}`;
      const prompt = cleanString(slot?.prompt);
      const source = cleanString(slot?.source).replace(/\\/g, "/");
      if (!prompt) errors.push(`scene[${sceneIndex}] (${parentId || "?"}).page_blueprint.slots[${slotIndex}]: falta prompt.`);
      if (!/^images\/cells\/[a-z0-9][a-z0-9_.-]*\.(?:jpg|jpeg|png|webp)$/i.test(source) || source.includes("..")) {
        errors.push(`scene[${sceneIndex}] (${parentId || "?"}).page_blueprint.slots[${slotIndex}]: source debe vivir en images/cells/.`);
      }
      cellIds.push(cellId);
      expanded.push({
        ...raw,
        ...
        type: "panel",
        render_mode: "static",
        references: slot?.references && typeof slot.references === "object" && !Array.isArray(slot.references)
          ? slot.references : (raw?.references || {}),
        visual: { image_prompt: prompt },
        voiceover: {},
        captions: {},
        editor_motion: { enabled: false, preset: "static", zoom: 1, pan: 0 },
        transition_in: "cut",
        __v6_page_cell: { parentSceneId: parentId, slotId, source },
      });
    }
    expanded.push({
      ...raw,
      id: parentId,
      scene_id: undefined,
      __v6_composition_only: { pageBlueprint: page, cellIds },
    });
  }
  return expanded;
}
...
export function parseProject(rawJson) {
  ...
  const isNovela = ["novela-coreana", "novelas-coreanas-eng"].includes(preset);
  ...
  const usesAssetGraph = isManhwa || (isNovela && (hasPoseGraph(charMap) || hasEscenarioGraph));
  ...
  const flowReferenceNames = {};
  const rememberFlowReferenceName = (asset, name) => {
    const key = outputIdentity(asset);
    if (key && cleanString(name)) flowReferenceNames[key] = cleanString(name).slice(0, 120);
  };
  if (Array.isArray(rawJson.ingredients)) {
    rawJson.ingredients.forEach((ing, k) => {
      ...
      const norm = {
        ...
        flowName: cleanString(ing?.display_name) || `${type === "character_edited" ? "Personaje" : type === "location_plate" ? "Escenario" : "Referencia"} — ${humanizeFlowToken(iid)}`,
      ...
      rememberFlowReferenceName(norm.outputFile, norm.flowName);
  ...
  if (usesAssetGraph) {
    const addGeneratedAsset = (id, prompt, outputFile, referenceAssets, flowName) => {
      ...
      const norm = { id, type: "manhwa_asset", base: null, prompt, outputFile, referenceAssets, flowName, imageUrl: null, imageFilePath: null };
      rememberFlowReferenceName(outputFile, flowName);
      ingredients.push(norm);
    ...
    for (const [cid, c] of Object.entries(charMap)) {
      ...
      for (const [pose, rawPose] of Object.entries(poses)) {
        ...
        rememberFlowReferenceName(def.asset, `Personaje — ${charDisplayName(cid)} — ${humanizeFlowToken(pose)}`);
      ...
      for (const [pose, rawPose] of Object.entries(poses)) {
        ...
        const refPose = def.referenceDisabled ? "" : (def.referenceKey || (pose !== "base" ? "base" : ""));
        ...
        addGeneratedAsset(`${assetGraphPrefix}_character_${cid}_${pose}`, def.prompt, def.asset, refs,
          `Personaje — ${charDisplayName(cid)} — ${humanizeFlowToken(pose)}`);
      }
    }
    ...
    for (const [eid, e] of Object.entries(escenarios)) {
      ...
      if (typeof e?.reference_asset === "string" && !views.base) {
        const scenarioName = cleanString(e?.display_name) || humanizeFlowToken(eid);
        rememberFlowReferenceName(e.reference_asset, `Escenario — ${scenarioName} — Base`);
      ...
      for (const [view, rawView] of Object.entries(views)) {
        ...
        const scenarioName = cleanString(e?.display_name) || humanizeFlowToken(eid);
        rememberFlowReferenceName(def.asset, `Escenario — ${scenarioName} — ${humanizeFlowToken(view)}`);
        manhwaEscenarioDefs.set(`${eid}:${view}`, def);
      }
      for (const [view, rawView] of Object.entries(views)) {
        const def = manhwaAssetDef(rawView);
        if (def.mode !== "generate") continue;
        const refs = isManhwaV7 ? [] : [...def.referenceAssets];
        // V7: una vista nueva NO hereda la placa base. reference_view solo se adjunta cuando el JSON
        // lo declara y sus camaras son compatibles; de otro modo Grok clona el angulo anterior.
        const refView = def.referenceDisabled ? ""
          : (isManhwaV7 ? def.referenceKey : (def.referenceKey || (view !== "base" ? "base" : "")));
        const refDef = refView ? manhwaEscenarioDefs.get(`${eid}:${refView}`) : null;
        if (refDef?.asset) {
          if (!isManhwaV7 || v7ScenarioCamerasCompatible(rawView?.camera_signature, views[refView]?.camera_signature)) {
            refs.push(refDef.asset);
          } else {
            errors.push(`escenarios.${eid}.views.${view}: reference_view "${refView}" tiene camera_signature incompatible; se rechazo para no clonar el angulo.`);
          }
        }
        const scenarioName = cleanString(e?.display_name) || humanizeFlowToken(eid);
        addGeneratedAsset(`${assetGraphPrefix}_escenario_${eid}_${view}`, def.prompt, def.asset, refs,
          `Escenario — ${scenarioName} — ${humanizeFlowToken(view)}`);
      }
    }
  }
  ...
  const runtimeRawScenes = expandManhwaRuntimeScenes(rawJson, rawScenes, errors);
  ...
  const allIds = runtimeRawScenes.map(sceneIdOf);
  const seenIds = new Set();
  const scenes = [];

  runtimeRawScenes.forEach((raw, i) => {
    const where = `scene[${i}]`;
    ...
    // Subconjunto alineado con references.characters. Flow necesita distinguir las poses de
    // personaje de los escenarios/props: las primeras se convierten en entidades persistentes de
    // su biblioteca "Characters", mientras que el resto sigue viajando como medio nombrado.
    ...
    if (usesAssetGraph && sceneType === "panel" && Array.isArray(references.characters) && references.characters.length) {
      ...
      for (const c of references.characters) {
        ...
        if (ref) {
          ...
          characterReferenceAssets.push(ref);
        } else {
          characterReferenceAssets.push("");
        }
      }
    ...
    const compositionCellIds = Array.isArray(raw?.__v6_composition_only?.cellIds)
      ? raw.__v6_composition_only.cellIds : [];
    if (compositionCellIds.length) {
      sceneRefs = [
        ...sceneRefs,
        ...compositionCellIds.map((sceneId) => ({ sceneId, useFor: ["page_composition"], strength: "required" })),
      ];
    }
    ...
    scenes.push({
      ...
      renderMode: (raw.render_mode === "animated" || (isNovela && usesAssetGraph)) ? "animated" : "static",
      ...
      narrationRef: raw?.narration_ref && typeof raw.narration_ref === "object"
        ? { unitId: cleanString(raw.narration_ref.unit_id), timingWeight: Number(raw.narration_ref.timing_weight) }
        : null,
      ...
      isPageCell: !!raw?.__v6_page_cell,
      pageParentId: cleanString(raw?.__v6_page_cell?.parentSceneId),
      pageSlotId: cleanString(raw?.__v6_page_cell?.slotId),
      pageCellSource: cleanString(raw?.__v6_page_cell?.source),
      compositionOnly: !!raw?.__v6_composition_only,
      pageBlueprint: raw?.__v6_composition_only?.pageBlueprint || null,
      pageCellIds: compositionCellIds,
      // runtime.
  ...
  const ttsExport = isManhwa
    ? {
      ...
      model_id: cleanString(rawTtsExport?.model_id) || cleanString(pipelineTts?.model_id) || MANHWA_DEFAULT_TTS_MODEL_ID,
      elevenlabs_speed: Number.isFinite(Number(rawTtsExport?.elevenlabs_speed)) && Number(rawTtsExport?.elevenlabs_speed) > 0
        ? Number(rawTtsExport.elevenlabs_speed)
        : MANHWA_DEFAULT_ELEVENLABS_SPEED,
      voice_settings: {
        ...MANHWA_DEFAULT_VOICE_SETTINGS,
        ...((rawTtsExport?.settings && typeof rawTtsExport.settings === "object") ? rawTtsExport.settings : {}),
        ...((rawTtsExport?.voice_settings && typeof rawTtsExport.voice_settings === "object") ? rawTtsExport.voice_settings : {}),
      },
    ...
    : preset === "novelas-coreanas-eng"
      ? { ...(rawTtsExport || {}), voice_id: ENGLISH_NOVELA_VOICE_ID }
      : rawTtsExport;
  ...
  const project = {
    ...
    // Identidad operativa separada para comparativas A/B. `seriesId` sigue siendo la raiz canonica
    // de assets; `comparisonSeriesId` solo aisla memoria/proyecto del proveedor sin copiar archivos.
    comparisonVariant: cleanString(rawProject.comparison_variant),
    comparisonSeriesId: cleanString(rawProject.comparison_series) || cleanString(rawProject.comparison_series_id),
    // Una comparativa limpia no puede heredar el proyecto que este abierto por flowReuseProject.
    // El primer arranque crea uno nuevo; despues la asociacion persistida de esta serie se reutiliza.
    forceNewFlowProject: rawProject.force_new_flow_project === true
      || cleanString(rawProject.flow_project_mode).toLowerCase() === "new",
    ...
    voiceSpeed: Number(rawJson.audio && rawJson.audio.voice_speed) || (isNovela ? 1.25 : 1),
    ...
    // Modelo exacto de imagen de Flow. El driver lo confirma en la UI antes de cada Enter para que
    // una preferencia heredada de otro proyecto no cambie silenciosamente Pro/2/Lite.
    imageModel: cleanString(rawJson?.pipeline?.image_generation?.model) || null,
    animationProvider: (() => {
    ...
    ingredients,
    // Nombre humano por archivo para que los uploads de Flow no queden como "Imagen generada".
    // Las claves usan slash normalizado/minusculas; el SW las alinea con localReferencePaths.
    flowReferenceNames,
    ...
    ttsExport,
    narrationTrack: rawJson.narration_track && typeof rawJson.narration_track === "object"
      && !Array.isArray(rawJson.narration_track) ? rawJson.narration_track : null,
    v7TimelineModel: cleanString(rawJson?.v7_contract?.timeline_model),

...
export function parseProject(rawJson) {
  ...
  if (Array.isArray(rawJson.ingredients)) {
    ...
    rawJson.ingredients.forEach((ing, k) => {
      ...
      if (!type || !INGREDIENT_TYPES.has(type)) {
        errors.push(`${w} (${iid}): 'type' invalido (usa character_edited|entity|location_plate|style_frame).`); return;
      }
  ...
  const imageOnly = animTool === "none" || (rawJson.project && rawJson.project.preset === "historias");
  ...
  rawScenes.forEach((raw, i) => {
    ...
    if (!animationPrompt && !imageOnly) errors.push(`${where} (${id ?? "?"}): falta 'visual.animation_prompt' (o 'animation.prompt').`);
  ...
  const project = {
    ...
    imageOnly,
    // velocidad de GENERACION de la voz en Fish (prosody.speed, 0.5-2.0). OPT-IN via audio.voice_speed.
    // historias: DEFAULT 0.95 (un pelin lento pero con TONO natural; el playbackRate del render queda en 1.0
    // para NO engravar la voz). Otros presets: 1.
    voiceSpeed: Number(rawJson.audio && rawJson.audio.voice_speed) || (rawProject.preset === "historias" ? 0.95 : 1),

...
export function createOrchestrator(deps) {
    ...
    while (scene.attempts <= maxRetries) {
      ...
      try {
        ...
        if (e?.noAutoRetry) {
          ...
          if (state.config.pauseOnFailure ?? DEFAULT_CONFIG.pauseOnFailure) await deps.effects.pauseForError(`escena ${scene.id}: ${reason}`, scene.id);
          return;
        }
  ...
  async function runQueue() {
    ...
    while (true) {
      ...
      let idx = nextSceneIndexByStatus(state.scenes, 0, target);
      ...
      if (processed && processed.status === statusBefore) {
        ...
        if (s3.config.pauseOnFailure ?? DEFAULT_CONFIG.pauseOnFailure) { await deps.effects.pauseForError(`escena ${processed.id}: el runner no avanzo`, processed.id); break; }
        continue;
      }

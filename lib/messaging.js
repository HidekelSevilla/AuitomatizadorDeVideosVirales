...
export const CMD = Object.freeze({
  ...
  RETRY_INGREDIENT: "cmd:retry_ingredient", // { ingredientId } regenera un ingrediente y sobrescribe su asset si aplica
...
export const ACT = Object.freeze({
  ...
  CREATE_CHARACTER_FROM_MEDIA: "act:create_character_from_media", // { name, mediaName } -> medio ya nombrado -> Character persistente
  ...
  PRELOAD_REFERENCES: "act:preload_references", // (Flow) sube/nombra la biblioteca local antes de generar escenas; no envia prompt
  PREPARE_IMAGE: "act:prepare_image",    // (Grok) selecciona Imagen antes de subir referencias; cambiar modo las borra
  GENERATE_IMAGE: "act:generate_image",  // { prompt, characterNames:[], sceneRefImageUrls:[], localReferencePaths:[], aspectRatio, count }
  COLLECT_IMAGE: "act:collect_image",    // (Grok) recupera la imagen actual en /imagine/post tras perder el canal
  ANIMATE: "act:animate",                // { prompt, model, aspectRatio }  (secuencial: dispara Y espera)
  PREPARE_VIDEO: "act:prepare_video",    // (Grok) selecciona Video antes de subir el still; cambiar modo borra adjuntos
  ...
  WAIT_FOR_REFS: "act:wait_for_refs",    // (Grok) confirma que CDP produjo File(s)/preview(s) antes de enviar
  IMAGE_KEYS: "act:image_keys",          // (Grok) huellas compactas de resultados visibles antes de generar
  OPEN_IMAGE: "act:open_image",          // (Grok) abre la primera variante nueva para adoptar el asset del post
...
// Prioridad normal: config.fishVoiceId (si lo pegas) > preset > DEFAULT_VOICE_ID.
// Un preset con forceVoice:true ignora cualquier override y usa siempre su voiceId.
...
export const FISH_PRESETS = Object.freeze({
  ...
  // novela-coreana: 1 sola voz (narradora calida) para todo. voiceover.speaker se ignora (Fase 1).
  ...
  // historias: narrador grave de documental (3a persona). Voz default del preset.
...
export const DEFAULT_VOICE_MODEL = "s2.1-pro";
...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  maxHourlyPauseMs: 180000,     // pausa MAXIMA al tope/hora (3 min); arranca ventana nueva tras la pausa
  grokPromptChunkThresholdChars: 3000, // hasta 3k conserva el pegado unico anterior
  grokPromptChunkChars: 480,    // sobre 3k escribe el prompt COMPLETO en bloques; nunca recorta contenido
  ...
  pauseOnFailure: true,         // imagenes: continua independientes + 1 pasada final y pausa al cierre; animacion: pausa al agotar reintentos.
  ...
  fishApiKey: "d4e19ba180a5403a9bd203c28def4d0f",  // HARDCODE local (uso personal): key de api.fish.audio. /secrets la sobreescribe si existe.
  fishVoiceId: "",              // reference_id de la voz (id de modelo; vacio = voz por defecto de Fish)
  fishModel: "s2.1-pro",        // modelo TTS de Fish: S2.1 Pro para toda generacion nueva
  ...
  remoteControlUrl: "http://localhost:35729/remote", // puente local para Telegram/control remoto
...
export function makeInitialState() {
  return {
    ...
    // Serie -> proyecto Flow confirmado. Persiste fuera de project para sobrevivir al cambio P1 -> P2.
    flowProjects: {},
    inspector: false,
  };
}
...
export async function send(message) {
  // Reintenta SOLO cuando Chrome confirma que el mensaje no tuvo receptor (worker reiniciandose).
  // No reintentamos un puerto cerrado despues de entregar: podria duplicar RUN_ALL/audio/animacion.
    ...
    const result = await new Promise((resolve) => {
      try {
        chrome.runtime.sendMessage(message, (resp) => {
          ...
          resolve({ resp: resp ?? null, error });
        });
      } catch (e) {
        resolve({ resp: null, error: e?.message ?? String(e) });
      }
    });
    if (result.resp != null) return result.resp;
    if (!/receiving end does not exist|could not establish connection/i.test(result.error)) return null;
  }
  return null;
}

export function resolveFishVoice(preset, configVoice = "") {
  const presetCfg = FISH_PRESETS[preset] || null;
  const presetVoice = pickPresetVoiceId(presetCfg);
  const cfgVoice = typeof configVoice === "string" ? configVoice.trim() : "";
  if (presetCfg?.forceVoice && presetVoice) {
    return { voiceId: presetVoice, source: `preset obligatorio "${preset}"`, presetCfg };
  }
  if (cfgVoice) return { voiceId: cfgVoice, source: "config", presetCfg };
  if (presetVoice) return { voiceId: presetVoice, source: `preset "${preset}"`, presetCfg };
  return { voiceId: DEFAULT_VOICE_ID, source: "DEFAULT (sin preset)", presetCfg };
}
...
// Divide texto para Input.insertText sin perder ningun caracter. Array.from evita cortar pares
// sustitutos (emoji/simbolos Unicode) entre bloques. La normalizacion de saltos ya era parte del
// contrato del compositor: Enter se reserva exclusivamente para enviar la generacion.
export function chunkTextForTrustedInput(text, chunkChars = 480) {
  const normalized = String(text ?? "").replace(/\r?\n/g, " ");
  const chars = Array.from(normalized);
  const size = Math.max(64, Math.floor(Number(chunkChars) || 480));
  const chunks = [];

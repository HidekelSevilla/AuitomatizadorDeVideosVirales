...
export function validateQueueProject(rawJson, options = {}) {
  ...
  if (usesAssetGraph) {
    ...
    for (const s of scenes) {
      ...
      // Grok solo acepta 3 imagenes de referencia por generacion. Cuenta: personaje(s) + assets(sistema_ui)
      // + escenario + escenas previas + ingredientes. Si se pasa, el SW recorta (ingredientes antes que
      // escenas), pero es sintoma de un JSON mal balanceado -> avisar para corregirlo en origen.
      const grokRefCount =
        (Array.isArray(s?.references?.characters) ? s.references.characters.length : 0)
        + (Array.isArray(s?.references?.assets) ? s.references.assets.length : 0)
        + (escRef.id ? 1 : 0)
        + (Array.isArray(s?.references?.scenes) ? s.references.scenes.length : 0)
        + ingredientRefIds(s?.references?.ingredients).length;
      if (grokRefCount > 3) errors.push(`${id}: ${grokRefCount} referencias exceden el limite de Grok (3). Prioriza ingredientes (personaje + escenario + sistema_ui) y elimina references.scenes; si aun son 4, deja 1 solo personaje en refs (describe al otro por posicion) o convierte la ventana en solo-resplandor.`);
    }

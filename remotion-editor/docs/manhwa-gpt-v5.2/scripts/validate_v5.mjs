...
const containerRe = /\b(capsule|capsula|cápsula|container|contenedor|vehicle|truck|car|room|chamber|cell|cockpit|cradle)\b/i;
...
const transparentContainerRe = /\b(?:transparent|transparente|clear (?:polycarbonate|canopy|shell)|see-through)\b[\s\S]{0,80}\b(?:capsule|capsula|cápsula|container|contenedor|chamber|cell|cockpit|cradle)\b|\b(?:capsule|capsula|cápsula|container|contenedor|chamber|cell|cockpit|cradle)\b[\s\S]{0,80}\b(?:transparent|transparente|clear (?:polycarbonate|canopy|shell)|see-through)\b/i;
const brokenPluralPossessiveRe = /\b[a-z][a-z-]*s's\b/gi;
const duplicatedChestCrackRe = /\b(?:single\s+)?(?:electric[- ]?)?(?:purple\s+)?chest crack(?:'s)?\s+(?:single\s+)?(?:electric[- ]?)?(?:purple\s+)?chest crack\b/gi;
const trueLongShotRe = /\b(?:extreme[- ]wide|very wide|deep wide|distant wide|panoramic wide|ultra[- ]wide|long shot|establishing long shot|monumental wide)\b/i;
const longDistance = "(?:1[2-9]|2[0-5]|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty(?:[- ](?:one|two|three|four|five))?)";
const distanceRe = new RegExp(`\\b(?:camera (?:is |placed |positioned )?(?:at least )?${longDistance}(?:\\.[0-9]+)? meters? (?:away|back)|${longDistance}(?:\\.[0-9]+)?-meter distance|from ${longDistance} meters? away)\\b`, "i");
const layersRe = /\bforeground\b[\s\S]{0,180}\bmidground\b[\s\S]{0,180}\bbackground\b|\bbackground\b[\s\S]{0,180}\bmidground\b[\s\S]{0,180}\bforeground\b/i;
...
const percentages = (prompt) => [...String(prompt || "").matchAll(/(\d{1,3})(?:\s*[–-]\s*(\d{1,3}))?\s*(?:%|percent)/gi)]
  .map((m) => ({ start: Number(m[1]), end: Number(m[2] || m[1]), index: m.index || 0 }));
const percentageNear = (prompt, nounRe, predicate) => {
  const text = String(prompt || "");
  for (const match of text.matchAll(nounRe)) {
    const start = Math.max(0, (match.index || 0) - 90);
    const end = Math.min(text.length, (match.index || 0) + match[0].length + 90);
    if (percentages(text.slice(start, end)).some(predicate)) return true;
  }
  return false;
};
const hasSubjectOccupancy = (prompt) => percentageNear(
  prompt,
  /\b(?:subject|human|person|character|figure|protagonist|cleaner|worker|prisoner|captain|child|boy|girl|creature|monster|dog|trio|pair)\b/gi,
  ({ start, end }) => start >= 10 && end <= 25,
);
const hasEnvironmentOccupancy = (prompt) => percentageNear(
  prompt,
  /\b(?:environment|setting|architecture|tunnel|street|city|landscape|surroundings|background)\b/gi,
  ({ start, end }) => Math.max(start, end) >= 65,
);
...
const trueLongShotRequired = panels.length < 20 ? 1
  : panels.length < 30 ? 2
    : panels.length < 40 ? 3
      : panels.length <= 50 ? 4
        : Math.ceil(panels.length * 0.10);
const trueLongShotFinalRequired = panels.length >= 40 ? 2 : panels.length >= 20 ? 1 : 0;
const finalFortyStart = Math.floor(panels.length * 0.6);
const trueLongShotChecks = panels.map((s, index) => {
  const prompt = promptOf(s);
  const checks = {
    long_shot_marker: trueLongShotRe.test(prompt),
    subject_occupancy_10_25: hasSubjectOccupancy(prompt),
    environment_occupancy_65plus: hasEnvironmentOccupancy(prompt),
    explicit_camera_distance: distanceRe.test(prompt),
    foreground_midground_background: layersRe.test(prompt),
  };
  return {
    scene_id: s.id,
    panel_index: index,
    in_final_40_percent: index >= finalFortyStart,
    ...checks,
    qualifies: Object.values(checks).every(Boolean),
  };
});
const qualifyingTrueLongShots = trueLongShotChecks.filter((x) => x.qualifies);
const qualifyingFinalFortyLongShots = qualifyingTrueLongShots.filter((x) => x.in_final_40_percent);
const trueLongShotPass = qualifyingTrueLongShots.length >= trueLongShotRequired
  && qualifyingFinalFortyLongShots.length >= trueLongShotFinalRequired;
...
const promptLengthChecks = panels.map((s, index) => {
  const prompt = promptOf(s);
  const words = promptWordCounts[index] || 0;
  const complex = compositeRe.test(prompt)
    || actionRe.test(prompt)
    || (s?.references?.characters?.length || 0) >= 2
    || (s?.references?.assets?.length || 0) >= 2;
  const reportThreshold = complex ? 110 : 90;
  return {
    scene_id: s.id,
    words,
    complexity: complex ? "complex" : "standard",
    report_threshold: reportThreshold,
    report: words > reportThreshold,
    blocks_release: words > 120,
  };
});
const promptLengthReports = promptLengthChecks.filter((x) => x.report);
const promptLengthBlocks = promptLengthChecks.filter((x) => x.blocks_release);

const brokenPromptLanguage = panels.flatMap((s) => {
  const prompt = promptOf(s);
  const issues = [];
  const possessives = [...prompt.matchAll(brokenPluralPossessiveRe)].map((m) => m[0]);
  const chestCrackDuplications = [...prompt.matchAll(duplicatedChestCrackRe)].map((m) => m[0]);
  if (possessives.length) issues.push({ type: "broken_plural_possessive", matches: possessives });
  if (chestCrackDuplications.length) issues.push({ type: "duplicated_chest_crack", matches: chestCrackDuplications });
  return issues.length ? [{ scene_id: s.id, issues }] : [];
});

const transparentContainerWithoutOccupant = panels
  .filter((s) => {
    const prompt = promptOf(s);
    const containerAssetIdentities = (s?.references?.assets || []).map((ref) => {
      const key = `${ref.id}/${ref.pose}`;
      return `${ref.id} ${displayNames.get(ref.id) || ""} ${posePrompts.get(key) || ""}`;
    }).filter((identity) => containerRe.test(identity));
    return containerAssetIdentities.length > 0
      && transparentContainerRe.test(`${prompt} ${containerAssetIdentities.join(" ")}`)
      && (s?.references?.characters?.length || 0) === 0;
  })
  .map((s) => s.id);
...
let fpsCheck = {
  checked: false,
  project_fps: Number(raw?.project?.fps || 0) || null,
  mismatch: false,
};
try {
  const slug = String(raw?.project?.slug || "");
  const videoFile = path.resolve(process.cwd(), "out", `${slug}.mp4`);
  if (slug && fs.existsSync(videoFile)) {
    ...
    const rate = String(probe?.streams?.[0]?.avg_frame_rate || probe?.streams?.[0]?.r_frame_rate || "0/1");
    const [numerator, denominator = "1"] = rate.split("/").map(Number);
    const videoFps = denominator ? numerator / denominator : 0;
    const projectFps = Number(raw?.project?.fps || 0);
    fpsCheck = {
      checked: true,
      video_file: videoFile,
      project_fps: projectFps || null,
      video_fps: Number(videoFps.toFixed(3)),
      mismatch: Boolean(projectFps && videoFps && Math.abs(projectFps - videoFps) > 0.01),
    };
  }
} catch (error) {
  fpsCheck = {
...
const promptPass = mechanicalPass
  ...
  && trueLongShotPass
  && breathRatio >= 0.2
  && promptLengthReports.length === 0
  && brokenPromptLanguage.length === 0
  && transparentContainerWithoutOccupant.length === 0
  && occupancyRisks.length === 0
  && unreferencedNamedActors.length === 0
  && maxCloseRun <= 2
...
const preflightGates = {
  ...
  full_script_exact: joined === fullScript,
  unique_prompts: duplicates.length === 0,
  scene_load: overlong.length === 0,
  shot_present: missing.shot.length === 0,
  angle_present: missing.angle.length === 0,
  time_present: missing.time.length === 0,
  true_long_shots: trueLongShotPass,
  breath_ratio_minimum: breathRatio >= 0.2,
  prompt_length_within_type_limit: promptLengthReports.length === 0,
  prompt_language_integrity: brokenPromptLanguage.length === 0,
  transparent_container_has_occupant_reference: transparentContainerWithoutOccupant.length === 0,
  occupancy_role_map: occupancyRisks.length === 0,
  no_unreferenced_named_actors: unreferencedNamedActors.length === 0,
  close_macro_run_max_two: maxCloseRun <= 2,
  action_pose_not_neutral: neutralActionPoseRisks.length === 0,
  tts_tags_english: nonEnglishTagRisks.length === 0,
  output_fps_matches_project: !fpsCheck.mismatch,
};
...
const report = {
  ...
  prompts: {
    ...
    length_gate: {
      standard_report_above_words: 90,
      complex_report_above_words: 110,
      hard_block_above_words: 120,
      reports: promptLengthReports,
      blocks: promptLengthBlocks,
      pass: promptLengthBlocks.length === 0,
    },
    true_long_shots: {
      definition: "Explicit long/extreme/deep/distant/monumental shot + subject 10-25% + environment >=65% + camera distance 12-25m + foreground/midground/background.",
      required_total: trueLongShotRequired,
      required_in_final_40_percent: trueLongShotFinalRequired,
      final_40_percent_begins_at_panel_index: finalFortyStart,
      qualifying_count: qualifyingTrueLongShots.length,
      qualifying_in_final_40_percent: qualifyingFinalFortyLongShots.length,
      candidates: trueLongShotChecks.filter((x) => x.long_shot_marker),
      qualifying: qualifyingTrueLongShots,
      pass: trueLongShotPass,
    },
    broken_language: brokenPromptLanguage,
    transparent_container_without_character_occupant_reference: transparentContainerWithoutOccupant,
  ...
  rhythm: {
    ...
    minimum_ratio_gate: 0.2,
    pass: breathRatio >= 0.2,
  ...
  render_output: { fps: fpsCheck },
  render_dependent: "RENDER_PENDING",
};

console.log(JSON.stringify(report, null, 2));
process.exit(report.preflight_status === "PROMPT_RELEASE" ? 0 : 1);

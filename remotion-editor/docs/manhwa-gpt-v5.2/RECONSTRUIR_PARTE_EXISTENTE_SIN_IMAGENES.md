# Reconstruir una Parte existente en V5.2 sin subir renders
...
```text
RECONSTRUIR PARTE V5.2 — RETAKE ESTRUCTURAL SIN CAMBIAR EL GUION

Fuentes en este orden:
1. STORY_PACKET_V5 migrado para canon y MONOLOGO_LOCKED.
2. Reglas V5.2 para producción.
...
- usar las escenas que exija el runtime; 1.3–4.5 s normales, amenaza/agencia/payoff dentro del timing budget;
- incluir 4–6 TRUE_LONG_SHOT y 20–28% de respiros reales;
...
Entrega JSON completo, assets nuevos, lista de reutilización/retakes y métricas V5.2.
```
...
No hay renders en esta etapa. Audita y repara el JSON completo con V5.2.
...
Repara hasta PROMPT_RELEASE; si no es posible, devuelve `BLOCKED_*` con causa. No declares RENDER_RELEASE.
```
